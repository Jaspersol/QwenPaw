...
describe("ModelSelector", () => {
  ...
  it("displays the global failover pointer when its chain is active", async () => {
    vi.mocked(providerApi.getFallbackModels).mockResolvedValue({
      models: [
        {
          provider_id: "openai",
    ...
    vi.mocked(providerApi.getActiveModels).mockResolvedValue({
      ...mockActiveModels,
      runtime_active_llm: { provider_id: "openai", model: "gpt-4" },
    });
  ...
  it("keeps a manual model outside the global fallback chain", async () => {
    vi.mocked(providerApi.getFallbackModels).mockResolvedValue({
      models: [{ provider_id: "openai", model: "gpt-4" }],
  ...
  it("uses the global pointer for backends without runtime_active_llm", async () => {
    vi.mocked(providerApi.getFallbackModels).mockResolvedValue({
      models: [{ provider_id: "openai", model: "gpt-4" }],
      current: { provider_id: "openai", model: "gpt-4" },
    });
