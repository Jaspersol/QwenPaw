...
import type { ActiveModelsInfo, FallbackModelsInfo } from "../../../api/types";
...
export default function ModelSelector({
  ...
  const [globalFallback, setGlobalFallback] =
  ...
  const refreshGlobalFallback = useCallback(async () => {
    try {
      setGlobalFallback(await providerApi.getFallbackModels());
    } catch {
      // Older backends do not expose the global compatibility chain.
    }
  }, []);

  useEffect(() => {
    void refreshGlobalFallback();
    const refresh = () => void refreshGlobalFallback();
    window.addEventListener("chat-turn-settled", refresh);
    window.addEventListener("model-switched", refresh);
    return () => {
      window.removeEventListener("chat-turn-settled", refresh);
      window.removeEventListener("model-switched", refresh);
    };
  }, [refreshGlobalFallback, selectedAgent]);
  ...
  const globalFallbackConfigured = Boolean(globalFallback?.models.length);
  const runtimeSlot = activeModels?.runtime_active_llm;
  const runtimeSlotInGlobalChain = Boolean(
    runtimeSlot &&
      globalFallback?.models.some(
        (slot) =>
  ...
  const displaySlot =
    globalFallbackConfigured && runtimeSlot && !runtimeSlotInGlobalChain
      ? runtimeSlot
      : globalFallbackConfigured && globalFallback?.current
      ? globalFallback.current
      : runtimeSlot ?? activeModels?.active_llm;
  const displayInGlobalChain = Boolean(
    displaySlot &&
      globalFallback?.models.some(
        (slot) =>
