...
class AgentBuilder:
    ...
    async def build(  # pylint: disable=too-many-statements,too-many-branches
        ...
        # Validate model availability. A configured global priority chain can
        # provide the default even when no legacy single active slot exists.
        ...
        if not has_active and not global_fallback_models:
        ...
        if active is not None and active.provider_id and active.model:
            ...
            model_slot_log = (
                f"fallback-chain[{len(global_fallback_models)}]"
                if global_fallback_models
                else "none"
            )
        _logger.info(
            ...
            agent_id,
            sanitize_log_value(model_slot_log),
