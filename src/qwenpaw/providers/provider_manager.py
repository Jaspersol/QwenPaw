...
class ProviderManager(
    ...
    def __init__(self) -> None:
        ...
        # Invalidates cached global fallback chains on provider mutations.
    ...
    # ------------------------------------------------------------------
    # Global priority-ordered default-model chain (compatibility surface)
    # ------------------------------------------------------------------
    ...
    def _load_fallback_models(self) -> List[ModelSlotConfig]:
        ...
        if not path.exists():
            return []
        try:
            with get_sync_path_lock(path), open(
                path,
                "r",
                encoding="utf-8",
            ) as handle:
    ...
    def get_fallback_models(self) -> List[ModelSlotConfig]:
        """Return a copy of the global default-model priority chain."""
        return list(self.fallback_models)

    def get_config_revision(self) -> int:
        """Return the revision used by the global chain cache."""
        return self._config_revision

    def set_fallback_models(self, models: List[ModelSlotConfig]) -> None:
        """Replace the global chain and reset its persistent pointer."""
        ...
        logger.info(
            "Updated global fallback model chain (%d entries): %s",
            len(normalized),
            ", ".join(
                f"{slot.provider_id}/{slot.model}" for slot in normalized
            )
            or "(empty)",
        )

    def get_fallback_models_info(self) -> List[Dict[str, str]]:
        """Return the global chain enriched with display names."""
