...
def _slot_in_chain(slot: Any, fallback_models: Sequence[Any]) -> bool:
    """Return whether a model slot belongs to the global priority chain."""
    return any(
        getattr(entry, "provider_id", "")
        == getattr(slot, "provider_id", "")
...
def _build_fallback_chain(
    ...
    *,
    ...
    thinking_level: str = "inherit",
) -> Tuple[Optional[ChatModelBase], Optional[Any]]:
    """Build the current branch's persistent global fallback chain."""
    from ..config.config import ModelSlotConfig
    from ..providers.fallback_chat_model import PersistentFallbackChatModel
    ...
    for value in fallback_models:
        ...
        if provider is None:
            logger.warning(
                "Global fallback provider '%s' was not found; skipping.",
        ...
        try:
            ...
            _install_model_formatter(raw_model, provider_id=provider_id)
        except Exception as exc:
            logger.warning(
                "Failed to build global fallback %s/%s: %s; skipping.",
                chain_slot.provider_id,
                chain_slot.model,
                exc,
            )
    ...
    if not chain_models:
        return None, None
    chain_model = PersistentFallbackChatModel(chain_models, chain_slots)
    return chain_model, chain_model.formatter


_fallback_chain_cache: dict[tuple[Any, ...], Any] = {}
_fallback_chain_cache_lock = threading.Lock()
_GLOBAL_FALLBACK_CHAIN_CACHE_MAX = 16
...
def _get_or_build_fallback_chain(
    ...
    *,
    ...
    key = (
        fingerprint,
        getattr(manager, "get_config_revision", lambda: 0)(),
        retry_config,
        rate_limit_config,
        compact_threshold,
        thinking_level,
    ...
    if cached is not None:
        return cached, cached.formatter
    ...
    if chain_model is not None:
        with _fallback_chain_cache_lock:
            _fallback_chain_cache[key] = chain_model
            while (
                len(_fallback_chain_cache)
                > _GLOBAL_FALLBACK_CHAIN_CACHE_MAX
            ):
                oldest_key = next(iter(_fallback_chain_cache))
                _fallback_chain_cache.pop(oldest_key)
...
def create_model_and_formatter(
    ...
    override_explicit = bool(
        slot is not None and slot.provider_id and slot.model,
    )
    if override_explicit:
        model_slot = slot

    manager = ProviderManager.get_instance()
    global_fallback_models = getattr(
        manager,
        "get_fallback_models",
        lambda: [],
    )()
    if global_fallback_models and not override_explicit:
        manual_override = bool(
            model_slot is not None
            and model_slot.provider_id
            and model_slot.model
            and not _slot_in_chain(model_slot, global_fallback_models),
        )
        if not manual_override:
            chain_model, chain_formatter = (
                _get_or_build_fallback_chain(
                    manager,
            ...
            if chain_model is not None and chain_formatter is not None:
                return chain_model, chain_formatter
            logger.warning(
                "Global fallback chain is configured but no slot could be "
                "built; continuing with the effective active model.",
            )
